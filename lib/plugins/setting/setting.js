(function (window) {
	...
	class Search {
		...
		constructor(setting) {
			...
			this.elements = {};
			this.elements.notfound = new window.Timeline.Plugin.Setting.Search.NotFound(this);
		}
	...
	class Li {
		...
		constructor(ul, matrix) {
			this.ul = ul;
			this.elements = {};
			this.elements.visibility = new window.Timeline.Plugin.Setting.Ul.Li.Visibility(this);

			let visibility = !matrix.hasOwnProperty(Timeline.Plugin.Setting.Ul.Li.Visibility.visibility())
				|| matrix[Timeline.Plugin.Setting.Ul.Li.Visibility.visibility()] === false;
	...
	class Ul {
		...
		constructor(setting) {
			...
			if (typeof window.Sortable === 'function') {
				...
				this.sortable = new Sortable.create(ul, {
					...
					handle: '.handle',
					reference: this,
					dataIdAttr: window.Timeline.Plugin.Setting.Ul.Li.data(),
					onEnd: function () {
		...
		*/
		...
		findLI(name) {
			let li = this.getLI();
			for (let item = 0; item < li.length; item++) {
				let data = li[item].out().getAttribute(window.Timeline.Plugin.Setting.Ul.Li.data());
				if (data === null
					|| data !== name) continue;
		...
		*/
		...
		addLI(structure) {
			let li = new window.Timeline.Plugin.Setting.Ul.Li(this, structure);
		...
		*/
		...
		update() {
			...
			for (let item = 0; item < setting.length; item++) {
				...
				map[setting[item].name][Timeline.Plugin.Setting.Ul.Li.Visibility.visibility()] = true;
			}
	...
	class Setting {
		...
		constructor(timeline) {
			...
			this.elements = {};
			this.elements.ul = new window.Timeline.Plugin.Setting.Ul(this);
			this.elements.search = new window.Timeline.Plugin.Setting.Search(this);
			this.elements.notice = new window.Timeline.Plugin.Setting.Notice(this);
		...
		*/
		...
		getK(name) {
			...
			for (let item = 0, k = 0; item < setting.length; item++) {
				if (true !== setting[item].hasOwnProperty(Timeline.Plugin.Setting.Ul.Li.Visibility.visibility())) continue;
				if (true !== setting[item][Timeline.Plugin.Setting.Ul.Li.Visibility.visibility()]) continue;
			...
			for (let item = 0; item < structure.length; item++)
				if (structure[item].name === name)
					return String.fromCharCode(35)
						+ structure[item][Timeline.Plugin.Setting.color()];
			return null;
		}
		...
		*/
		...
		request() {
			...
			for (let item = 0; item < list.length; item++) {
				...
				setting.push({
					name: list[item],
					[Timeline.Plugin.Setting.Ul.Li.Visibility.visibility()]: visibility
				});
			}
